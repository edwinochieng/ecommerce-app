...
export default function ShippingScreen() {
  ...
  const submitHandler = ({ fullName, address, city, postalCode, country }) => {
    ...
    Cookies.set(
      ...
      JSON.stringify({
        ...cart,
        ShippingScreen: { fullName, address, city, postalCode, country },
      })
    );
      ...
      >
        ...
        <div className='mb-4'>
          ...
          <input
            className='w-full border rounded-md p-2 outline-none ring-indigo-300 focus:ring'
            autoFocus
            {...register("city", { required: "Please enter Postal Code" })}
          />
          {errors.city && (
        ...
        <div>
          <button className='w-24 h-10 rounded bg-rose-500 text-white font-semibold text-base '>
            Next
          </button>
        </div>
      </form>
